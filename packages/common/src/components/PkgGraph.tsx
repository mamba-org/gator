  ...
  private async _updatePackages(): Promise<void> {
    try {
      ...
      if (available[this.props.package] === null) {
        ...
        error = (
          <span>
            Package dependencies could not be resolved. This may be a pip
            package or the package is not available in conda channels.
          </span>
        );
      } else {
      ...
      if (error instanceof Error && (error as any).message !== 'cancelled') {
