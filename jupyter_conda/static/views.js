define(["jquery", "base/js/utils", "./common", "./models"], function(
  $,
  utils,
  common,
  models
    ...
    refresh: function(data) {
      ...
      $.each(data, function(index, row) {
        ...
        $.each(that.columns, function(index, column) {
          var $cell = $("<div/>").addClass("col-xs-" + column.width);
  ...
  function new_env_prompt(callback) {
    ...
    var dialogform = $("<div/>")
      .attr("title", "Create New Environment")
      .append(
  ...
  $.extend(EnvView, {
    ...
    transforms: {
      name: function(row) {
        return common.link("#", row.name).click(function() {
          models.environments.select(row);
        });
      },

      is_default: function(row) {
        return $('<div class="default_col">').append(
          common.icon(row.is_default ? "check" : "")
        );
      },
      ...
      _action: function(row, $row) {
        ...
        return $('<span class="action_col"/>')
          ...
          .append(
            common.link(
              models.environments.export(row),
              common.icon("external-link")
            )
          )
          .append(
            common.icon("copy").click(function() {
              common.prompt(
                "Clone Environment",
                'Create a copy of "' + row.name + '"',
                "New name:",
                "Clone",
                function(new_name) {
          ...
          .append(
            common.icon("trash-o").click(function() {
              var msg =
                'Are you sure you want to permanently delete environment "' +
                row.name +
                '" ?';
    ...
    bindings: {
      ...
      "#install": function() {
        ...
        var msg =
          ...
          models.environments.selected.name +
          '" ?';
  ...
  $.extend(InstalledView, {
    ...
    update_label: function(count) {
      $(this.selector)
        .find(".toolbar_info")
        .text(
          common.pluralize(count, this.label) +
            ' in environment "' +
            models.environments.selected.name +
            '"'
        );
